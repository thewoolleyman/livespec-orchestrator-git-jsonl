...
`notes` are required-on-write but optional-on-read (legacy records
authored before each field's introduction read back as `null` without
firing a schema violation).
