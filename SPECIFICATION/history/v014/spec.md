# spec.md — livespec-orchestrator-git-jsonl

This is the natural-language specification for `livespec-orchestrator-git-jsonl`,
...
`livespec-orchestrator-git-jsonl.compat` block.
...
work-item at any moment is the LAST record in the JSONL file
...
realization", `livespec-orchestrator-git-jsonl` realizes the
...
- Files live at paths configured in the consumer project's
  `.livespec.jsonc` under the `livespec-orchestrator-git-jsonl` section.
  Default path: `work-items.jsonl` at the project root. The default
  MAY be overridden via the configuration block.
