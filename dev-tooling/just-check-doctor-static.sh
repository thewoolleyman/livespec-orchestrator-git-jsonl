...
if [ -z "$core_root" ]; then
    ...
    # installed_plugins.json[...]["livespec@livespec"][0] -- that per-project
    # list is unordered and its first row can be a different, stale project on a
    # mixed-build host, which the c1k9 currency gate then correctly blocks.
    core_root="$(
